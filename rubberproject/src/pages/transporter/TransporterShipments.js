    ...
    <div className={styles.container}>
      ...
      <div className="mb-4">
        ...
        <p className={styles.subtitle}>
          View all shipments packed by sellers and submit your transportation quote.
        </p>
      </div>
      ...
      ========================= */}
      {openShipmentsLoading && (
        <div className="alert alert-info border-0 shadow-sm">Loading shipments...</div>
      )}

      {openShipmentsError && (
        <div className="alert alert-danger border-0 shadow-sm">{openShipmentsError}</div>
      )}
            ...
            <table className={`table align-middle mb-0 ${styles.tableMinWidth}`}>
              <thead className={styles.tableHeader}>
                <tr>
                  ...
                  <th className={styles.thText} style={{ width: "37%" }}>Quote Submission</th>
                </tr>
              </thead>
              <tbody style={{ borderTop: "none" }}>
                {openShipments.map((item) => {
                  ...
                  const alreadySubmitted = myQuotes.some(
                    (quote) => quote.shipmentId === shipmentId,
                  );
                        ...
                        <div className="d-flex align-items-start gap-2">
                          ...
                          <div>
                            ...
                            <div className={styles.hsnText}>
                              Transport HSN: {item.shipment?.transportHSNCode || "N/A"}
                            </div>
                          </div>
                        </div>
                      </td>
                        ...
                        {alreadySubmitted ? (
                          ...
                          <div className="row g-2 align-items-start">
                            <div className="col-md-3">
                              <span className={styles.inputLabel}>Transport Price (₹)</span>
                              <div className="input-group input-group-sm">
                                <span className={`input-group-text bg-white border-end-0 text-muted ${styles.inputField}`}>₹</span>
                                <input
                                  type="number"
                                  placeholder=""
                                  value={quoteData[shipmentId]?.quotedPrice || ""}
                                  onChange={(e) => handleQuoteChange(shipmentId, "quotedPrice", e.target.value)}
                            ...
                            <div className="col-md-3">
                              <span className={styles.inputLabel}>Delivery Days</span>
                              <input
                                type="number"
                                placeholder="Enter days"
                                value={quoteData[shipmentId]?.estimatedDeliveryDays || ""}
                                onChange={(e) => handleQuoteChange(shipmentId, "estimatedDeliveryDays", e.target.value)}
                                className={`form-control form-control-sm ${styles.inputField}`}
                              />
                            </div>

                            <div className="col-md-4">
                              <span className={styles.inputLabel}>Additional Notes</span>
                              <textarea
                                placeholder="Enter additional notes"
                                value={quoteData[shipmentId]?.note || ""}
                                onChange={(e) => handleQuoteChange(shipmentId, "note", e.target.value)}
                            ...
                            <div className="col-md-2 pt-4">
                              <button
                                ...
                                disabled={submitQuoteLoading && activeShipmentId === shipmentId}
                              >
                                {submitQuoteLoading && activeShipmentId === shipmentId ? (
