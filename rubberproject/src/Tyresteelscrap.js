...
const Tyresteelscrap = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const steelItems = scrapRes.data.scrap_items.filter(item => item.type === 'Tyre steel scrap');
  ...
  const displayedItems = useMemo(() => {
    ...
    return items;
  }, [steelScrapItems, selectedProducts, selectedLocations, priceFilterActive, priceRange, approvalData]);
  ...
  const NextArrow = ({ onClick }) => <div className="custom-arrow custom-arrow-next" onClick={onClick}>❯</div>;
  const PrevArrow = ({ onClick }) => <div className="custom-arrow custom-arrow-prev" onClick={onClick}>❮</div>;
  ...
  return (
    <div className="tyre-scrap-layout mt-5">
      ...
      <p className="tyre-scrap-description">
        Tyre Steel Scrap is a high-quality byproduct of tyre recycling, primarily used in industrial applications such as construction and
         manufacturing. Products like **Pyro Steel** and **Rubber Crum Steel** are valuable raw materials for various engineering and 
         industrial processes.
         These materials are sustainably sourced, making them a cost-effective and eco-friendly choice for businesses.
      </p>
