  ...
  return (
    <div className={styles.productCard}>
      ...
      <div className={styles.productLeft}>
        <h3 className={styles.cardTitle}>Product in this Shipment</h3>
      </div>
      ...
      <div className={styles.tableRow}>
        ...
        <div>{matchedItem?.loadingLocation || "-"}</div>

        <div>{shipment?.shipmentFrom || "-"}</div>

        <div>{shipment?.shipmentTo || "-"}</div>
        <div>{matchedItem?.requiredQuantity || 0}</div>

        <div>{shipment?.shippedQuantity || 0}</div>

        <div>₹ {pricePerMT.toLocaleString()}</div>
